...
{
    uint32_t dest_slot, dest_lba, seq, need_gen;
    tape_result rc;
    ...
    /* §4.5's tape_reset_side_b row: one `sequence`, plus one `sb_generation`
       if stage clearing applies. */
    ...
    /* tapefs §8: after the preconditions, before the first index write. */
    if (need_gen != 0u) {
