...
/* Little-endian writers, the exact mirrors of tape_rd32/tape_rd64. Explicit
   byte assembly for the same reason the readers use it: the bytes a commit
   produces must not depend on the host. */
void tape_wr32(unsigned char *p, uint32_t v);
void tape_wr64(unsigned char *p, uint64_t v);
...
/*
 * §8's stage clearing, written as §4.6's partner-first superblock update:
 * promote_stage = 0, promote_staging_chunk = 0, a_high_water UNCHANGED,
 * sb_generation + 1, partner then candidate, flushing after each.
 *
 * Callers are tape_arm and tape_reset_side_b, after their own preconditions
 * and before their first index or chunk write. It is a no-op path on
 * promote_stage == 0 and is therefore not reached by ordinary media.
 */
