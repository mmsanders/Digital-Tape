 ...
 *
 * §8 STAGE CLEARING IS NOT IMPLEMENTED HERE EITHER. §8 states it as a rule ON
 * tape_arm and tape_reset_side_b, but performing it means writing a superblock
 * by §4.6, which is promote-recovery behaviour that neither accepted VT8-001
 * observation covers. Rather than commit an index onto stage-1 media — which §8
 * forbids, and whose consequence §8 spells out — both calls REFUSE on
 * promote_stage == 1 with zero writes. TAPE_ERR_BUSY is the closest §2 code and
 * is an invented semantic: PM's to dispose of, exactly like the mode boundary
 * above. Nothing in this candidate can SET promote_stage, since tape_promote,
 * tape_format and tape_dup are all undefined; stage-1 media can only arrive
 * pre-existing.
 */
...
tape_result tape_arm(tape *t, tape_rec_mode mode)
{
    ...
    /* NARROWING, NOT A SPEC CLAIM — see the coverage-boundary note above.
       tapefs §8 requires tape_arm to CLEAR a stale promote_stage before its
       first index or chunk write. Clearing it is a §4.6 superblock update, and
       this split implements no promote behaviour, so it refuses instead of
       committing an index onto stage-1 media, which §8 forbids outright. Zero
       writes. Both accepted VT8-001 observations are promote_stage == 0, so
       this branch is not exercised by either. */
    ...
    /* §4.5's tape_arm row: one `sequence` for the commit this arm authorises.
       No sb_generation is consumed, because nothing here writes a superblock.
       Every arm refusal above writes nothing, which is why the headroom test
       comes after them. */
