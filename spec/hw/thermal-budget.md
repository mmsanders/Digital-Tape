...
**Revision:** 0.6, 12 Sep 2026 · **Applies to:** production board rev A
...
**Part bound: TI CD74HC221E (16-DIP, bench) / CD74HC221M96 (16-SOIC, board)** — DigiKey catalogue, 12 Sep 2026: CD74HC221E $1.08/1, 1430 in stock; CD74HC221M96 $0.87/1, 4828 in stock; both listed 2-6 V. The supply envelope is sourced (2…6 V, covering 3.3 V); **its guaranteed pulse-width table at this rail is not** — that host is blocked, and a distributor's supply-range field is not a timing guarantee. A half sets the pulse (R = 143 kΩ, C = 100 nF **C0G**); B half sets the inhibit (R = 557 kΩ, C = 1 µF **film**). Both resistors are inside the family's specified 2…1000 kΩ range; the original 450 ms lockout on 470 nF needed **1368 kΩ** and was not.
...
- the ±14 % one-shot timing term is ASSUMED, not guaranteed: it is extrapolated from a 700 µs datasheet test point at VCC = 5 V and applied to a 390 ms interval at 3.3 V with a different R/C. Part now bound (TI CD74HC221E (16-DIP, bench) / CD74HC221M96 (16-SOIC, board), 2...6 V), but its guaranteed pulse-width table at 3.3 V is still unread -- every host serving that datasheet is blocked
...
- **Egress reaches the distributor but not the datasheet** (H-02, narrowed 12 Sep 2026). `www.digikey.com` resolves, which is how the part above got bound, priced and stock-checked. Every host that actually serves the pulse-width table refused: `ti.com`, `mm.digikey.com`, `rocelec.widen.net`, `nexperia.com`. So the remaining gap is narrower and more specific than "no vendor access": it is the **guaranteed pulse-width limits at 3.3 V over the intended R/C and temperature**, and nothing else. **A bench measurement is still the honest route** and does not wait on anyone: it belongs with WP-04's pulse measurement, on the same bench, on the same day. The method is specified in `hardware/measurements/solenoid-timing-method.md`.
...
| Rev | Date | Change |
|---|---|---|
| 0.6 | 2026-09-12 | **§6: the one-shot is bound to an orderable part, and the IR-018-16 gap is narrowed rather than closed.** The candidate is now TI `CD74HC221E` (16-DIP, bench) / `CD74HC221M96` (16-SOIC, board), catalogued at **2–6 V** — an envelope that covers the 3.3 V rail — with dated distributor price and stock in `hardware/sourcing/2026-09-12-parts.md`. **What is still not bound is the guaranteed pulse-width table at 3.3 V**: the distributor is reachable from the lead's environment but every host serving that datasheet is blocked, so `TIMING_BOUND_VERIFIED` stays `False`, the ±14 % term stays ASSUMED and **the verdict stays PROVISIONAL**. The H-02 bullet is corrected from "reaches no datasheet" to the narrower, accurate statement. A new criterion fails if the bound part's supply envelope does not contain `V_LOGIC`, prompted by a real catalogue listing of an HC part number at 4.5–5.5 V; it has a negative control. A bench method that needs **no coil, no cell and no fabricated board** is proposed in `hardware/measurements/solenoid-timing-method.md`. **No acceptance, and no change to the fabrication gate, which stays CLOSED.** |
