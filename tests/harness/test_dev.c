...
int main(void)
{
    ...
    if (tape_dev_file_create(&f, &dev, path, IMG_BLOCKS) != TAPE_OK) {
    ...
    fill(w, 0x11);
    CHECK_EQ_INT(dev.write(dev.ctx, 5u, 1u, w), TAPE_OK);
    CHECK_EQ_INT(dev.flush(dev.ctx), TAPE_OK);
    memset(r, 0, sizeof r);
    CHECK_EQ_INT(dev.read(dev.ctx, 5u, 1u, r), TAPE_OK);
    CHECK_MEM_EQ(r, w, TAPE_BLOCK_SIZE);

    /* A freshly created image is zero-filled, not garbage. */
    CHECK_EQ_INT(dev.read(dev.ctx, 6u, 1u, r), TAPE_OK);
    {
    ...
    CHECK_EQ_INT(dev.read(dev.ctx, IMG_BLOCKS - 1u, 1u, r), TAPE_OK);
    CHECK_EQ_INT(dev.read(dev.ctx, IMG_BLOCKS, 1u, r), TAPE_ERR_RANGE);
    CHECK_EQ_INT(dev.read(dev.ctx, IMG_BLOCKS - 1u, 2u, r), TAPE_ERR_RANGE);
    CHECK_EQ_INT(dev.write(dev.ctx, IMG_BLOCKS - 1u, 2u, w), TAPE_ERR_RANGE);
    CHECK_EQ_INT(tape_dev_file_close(&f), TAPE_OK);

    /* --- guardrail 06: read-only has no write function at all --- */
    CHECK_EQ_INT(tape_dev_file_open(&f, &dev, path, 0), TAPE_OK);
    CHECK(dev.write == NULL);
    CHECK_EQ_INT(dev.read(dev.ctx, 5u, 1u, r), TAPE_OK);
    CHECK_MEM_EQ(r, w, TAPE_BLOCK_SIZE);
    ...
    CHECK_EQ_INT(tape_dev_file_open(&f, &dev, path, 1), TAPE_OK);
    {
        struct tape_dev_sim sim;
        struct tape_dev sd;
        ...
        fill(w, 0x22);
        CHECK_EQ_INT(sd.write(sd.ctx, 10u, 1u, w), TAPE_OK);
        CHECK_EQ_INT(sd.write(sd.ctx, 11u, 1u, w), TAPE_OK);
        ...
        CHECK_EQ_INT(dev.read(dev.ctx, 11u, 1u, r), TAPE_OK);
        CHECK_MEM_EQ(r, w, TAPE_BLOCK_SIZE);
        CHECK_EQ_INT(dev.read(dev.ctx, 12u, 1u, r), TAPE_OK);
        {
            unsigned char zero[TAPE_BLOCK_SIZE];
            memset(zero, 0, sizeof zero);
            CHECK_MEM_EQ(r, zero, TAPE_BLOCK_SIZE);
    ...
    /* --- simulator: torn write --- */
    {
        ...
        fill(before, 0x33);
        CHECK_EQ_INT(dev.write(dev.ctx, 20u, 1u, before), TAPE_OK);
        ...
        CHECK_EQ_INT(dev.read(dev.ctx, 20u, 1u, r), TAPE_OK);
    ...
    /* --- SD stub: honest failure, and slot 0 has no write function --- */
    {
        ...
        struct tape_dev sd_dev;
        CHECK_EQ_INT(tape_dev_sd_bind(&sd_st, &sd_dev, 0u), TAPE_ERR_NOT_IMPLEMENTED);
        CHECK(sd_dev.write == NULL);                    /* source slot */
        CHECK_EQ_INT(tape_dev_sd_bind(&sd_st, &sd_dev, 1u), TAPE_ERR_NOT_IMPLEMENTED);
        CHECK(sd_dev.write != NULL);                    /* work slot */
        CHECK_EQ_INT(sd_dev.read(sd_dev.ctx, 0u, 1u, r), TAPE_ERR_NOT_IMPLEMENTED);
    }
