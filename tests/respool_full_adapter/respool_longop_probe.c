...
static bool prove_continuation(tape *t)
{
    bool more = false;
    uint64_t b, a;
    tape_result r = continue_respool(t, 1u, &more, &b, &a);
    return r == TAPE_OK && more && a > b;
}
...
static void emit_small_budget(void)
{
    ...
    do {
        before = g_ops;
        more = false;
        r = tape_respool(t, 1024u, &more);
        after = g_ops;
        if (calls != 0u) putchar(',');
        printf("{\"result\":\"%s\",\"block_budget\":1024,\"more_work\":%s,"
               "\"operation_token\":\"R2\",\"progress_before\":%llu,\"progress_after\":%llu}",
...
static void emit_zero_budget(void)
{
    ...
    printf("{\"phase\":\"initiation\",\"block_budget\":0,\"result\":\"%s\","
           "\"block_ops\":%llu,\"state_before\":\"idle\",\"state_after\":\"idle\"}",
           rn(r), (unsigned long long)(after - before));

    (void)fresh(&t);
    sr = start_respool(t, &more, &ib, &ia);
    before = g_ops;
    r = tape_respool(t, 0u, &more);
    after = g_ops;
    printf(",{\"phase\":\"continuation\",\"block_budget\":0,\"result\":\"%s\","
           "\"block_ops\":%llu,\"state_before\":\"%s\",\"state_after\":\"%s\"}",
           rn(r), (unsigned long long)(after - before),
           sr == TAPE_OK ? "R3" : "idle",
           sr == TAPE_OK ? "R3" : "idle");
    putchar(']');
...
static void emit_busy_then_continue(void)
{
    tape *t = NULL;
    bool more = false;
    uint64_t ib, ia, before, after, cb, ca;
    tape_result sr, busy, cont;
    (void)fresh(&t);
    sr = start_respool(t, &more, &ib, &ia);
    before = g_ops;
    busy = tape_seek(t, 0u);
    ...
    printf("{\"busy\":{\"result\":\"%s\",\"block_ops\":%llu,"
           "\"operation_token_before\":\"%s\",\"operation_token_after\":\"%s\"},"
           "\"continuation\":{\"result\":\"%s\",\"operation_token\":\"R4\","
           "\"progress_before\":%llu,\"progress_after\":%llu}}",
           rn(busy), (unsigned long long)(after - before),
           sr == TAPE_OK ? "R4" : "NONE", sr == TAPE_OK ? "R4" : "ENDED",
