...
def main() -> int:
    ...
    # Identities are verified against git history before any case runs.
    try:
        ids = provenance.verify()
    except provenance.ProvenanceError as exc:
        raise SystemExit(f"provenance: {exc}")
    verifier_tree = ids["verifier_tree"]
            ...
            for case in case_stream(args.fixture, args.pass_name, args.mode, start, end):
                ...
                try:
                    ...
                    summary = {
                        ...
                        "product_tree": tree,
                        "product_base": ids["product_base"],
                        "verifier_import_commit": ids["verifier_import_commit"],
    ...
    provenance.check_emitted(summary)
    (evidence / "summary.json").write_text(
        json.dumps(summary, indent=2, sort_keys=True) + "\n"
    )
    print(
