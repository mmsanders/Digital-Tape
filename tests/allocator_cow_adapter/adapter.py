...
def cpu_model() -> str:
    ...
    if p.is_file():
        ...
        for line in p.read_text(errors="replace").splitlines():
            ...
            key = key.strip().lower()
            value = value.strip()
            if value and key not in rows:
                rows[key] = value
        for key in ("model name", "hardware", "cpu model", "machine"):
            if rows.get(key):
                return rows[key]
        processor = rows.get("processor", "")
...
def main() -> int:
    ...
    timing_path = os.environ.get("WP07_TIMING_EVIDENCE")
    timing = {
        "format": "WP07-RESET-TIMING-EVIDENCE-1",
        "timing_environment": timing_environment(),
        "reset_stress": None,
        "ordinary_resets": [],
    }
    ...
    for raw in sys.stdin:
        try:
            ...
            if kind == "reset_stress":
                ...
                timing["timing_environment"] = obs.get(
                    "timing_environment", timing["timing_environment"]
                )
                timing["reset_stress"] = {
                    "elapsed_ns": obs.get("elapsed_ns"),
                    "timed_out": obs.get("timed_out"),
                    "write_events": obs.get("write_events"),
                }
            elif kind == "sequence":
                plan = cmd.get("plan")
                ...
                for action_index, action in enumerate(obs.get("actions", [])):
                    if isinstance(action, dict) and action.get("kind") == "reset":
                        timing["ordinary_resets"].append(
                            {
                                "seq_index": obs.get("seq_index"),
                                "action_index": action_index,
                                "elapsed_ns": action.get("elapsed_ns"),
                                "timed_out": action.get("timed_out"),
                                "write_events": action.get("write_events"),
                            }
                        )
            else:
