...
def main() -> int:
    ...
    timing_path = evidence / "reset-timing.json"
    run_env = os.environ.copy()
    run_env["WP07_TIMING_EVIDENCE"] = str(timing_path)
    ...
    provenance = {
        ...
        "reset_timing_sha256": sha256_file(timing_path) if timing_path.is_file() else "MISSING",
    ...
    print(f"reset_timing_sha256={provenance['reset_timing_sha256']}")
    if timing_path.is_file():
        timing = json.loads(timing_path.read_text(encoding="utf-8"))
        elapsed = [
            row["elapsed_ns"] for row in timing.get("ordinary_resets", [])
            if isinstance(row.get("elapsed_ns"), int)
        ]
        print(f"ordinary_reset_timing_count={len(elapsed)}")
        print(f"ordinary_reset_min_ns={min(elapsed) if elapsed else 'MISSING'}")
        print(f"ordinary_reset_max_ns={max(elapsed) if elapsed else 'MISSING'}")
        print("timing_environment=" + json.dumps(
            timing.get("timing_environment", {}), sort_keys=True
        ))
