...
Hardware Lead · **Amends:** ADR-105

**Decision.** The shell is retained by an interference clasp with a **continuous seam and
discontinuous engagement**: a tongue-and-groove parting line runs the whole perimeter, and the
retaining interference lives only on the two long walls, stopping 9 mm short of each corner. It
is opened with a **0.9 mm blade slot on the back face**, mid-run. There is no screw, and there is
no fingernail-accessible feature anywhere on the shell.

Working point: 0.30 mm total interference shared across both halves, 1.2 mm retention wall over
a 6.0 mm span, retention face at 40° from the pull axis. **0.75 % peak strain while opening,
0.086 % at rest.** A TPU lip seal supplies ~22 N of perimeter preload and absorbs the print
tolerance. Full derivation: `spec/hw/cartridge-shell.md`, generated by `hardware/mech/clasp.py`.

**ADR-105 said "serviceable, not tool-free: an adult with a driver may open the shell."** The
driver is now a spudger or a guitar pick, and the requirement it was protecting — *where a
serviceable shell and a curious six-year-old fight, the six-year-old wins* — is unchanged and is
met by a different mechanism.

**Rationale.** Three findings, in the order they mattered.

**A continuous perimeter lip does not work on a rectangular box.** It is the right instinct
about spreading load and it fails at the corners: a rectangle cannot open at a corner by
bending, only by stretching, and stretching is rigid. Running the lip through the corners
produces a joint that breaks rather than releases. Splitting seam from engagement keeps the
near-invisible line Michael asked for *and* gives the mechanism two long compliant runs.

**Creep is designed out, not tolerated.** The groove is deeper than the bead is proud, so the
wall returns undeflected and the joint holds 0.086 % strain for years instead of 0.75 %. That is
the whole answer to "does PETG creep", and it is a geometric claim, so
`hardware/cad/cartridge/test_shell.py` proves it with a boolean on the closed assembly rather
than with a paragraph.

**Retention force does not separate a child from an adult, and the design does not pretend it
does.** A determined seven-year-old with something to grip is within a factor of two of this
joint. What separates them is that there is *nothing to grip* — the seam is flush, so the only
force a child can bring is a pinch, and a pinch is an order of magnitude weaker than a grip. The
consequence is a rule rather than a number: **any feature giving a fingernail purchase on the
parting line spends the entire safety margin**, which rules out the recessed thumb-notch every
battery cover uses. The parent's low-force path is geometric instead — a blade unzips one 14 mm
span where a pull must deflect both full runs at once, about 10 : 1.

**Cost to reverse.** Moderate and falling. The variants are on the WP04-01 plate, so the
interference is still a swept parameter and not a commitment. Reversing to a screw after WP-24
costs the seam, the tooling for a boss, and Michael's stated preference; reversing to the sealed
cartridge costs the whole of `spec/hw/cartridge-shell.md` §10's argument and makes a dead card
into landfill. **The cartridge's outer dimensions are not part of this decision** — strain
contains no length term, so Michael can change the size on taste without reopening any of it.

---

## ADR-118 — Safety and thermal results ship as audit records, not as numbers

**Date:** 2026-09-05 · **Decided by:** Michael (issue #8, via PM Decisions 006 §5), format by
Hardware Lead

**Decision.** Every measurement backing a safety limit is recorded to
`hardware/measurements/TEMPLATE.md`: the criterion quoted verbatim, the procedure, instruments
with serials and calibration, conditions, **every raw reading in the order taken**, the
derivation with its arithmetic, the result against its limit, **the uncertainty**, and what
would change the result. Applies to WP-37 (thermal), WP-19 (the 85 dB cap), WP-24 S-4 (the
cartridge pinch test), and in adapted form to WP-27.

**Rationale.** Michael witnesses; the Verification Lead audits the method and the raw data
without being present. Two checks, neither of them the person who built the thing — which is
CLAUDE.md §2's "nobody grades their own homework" applied to the one place it was still
violated: for WP-37 and WP-19, the person proposing the criterion, running the test and
reporting the result would all have been me.

**An audit at a distance is a constraint on format, and it binds before the measurement rather
than after.** The auditor cannot see where a thermocouple sat or tell a settled reading from a
drifting one. Most sharply: `thermal-budget.md` §4's worst sustained case lands **0.2 K from the
JEITA ceiling**, and a result reported without its uncertainty makes that margin unauditable —
nobody can tell a 0.2 K margin from a 0.2 K instrument error. The two sections most likely to be
left thin, uncertainty and what-would-change-this, are the two the audit actually needs.
