...
| WP-04 | Transport spike: Route A vs Route B | You | hardware | Rev-6 source packet reproducibly splits WP-24 and WP-04 onto two nominal-A1 plates with 5 mm XY margins. PR #92 source controls are Hardware-authored and await independent audit; usable area/process and every physical result remain unproved, so printing is held |
| WP-05 | Parts order #1 | You | hardware | Five stored-rate vectors are independently arithmetically reproduced. Verification rejects PR #87's next-acquisition method under `P1-R21-V01` for schema/type/timestamp/metadata defects. A-2, exact identity, attribution, atomicity and end-to-end acceptance remain open |
...
| WP-07 | Chunk allocator, copy-on-write Side B | Agent | 1 | Exact reset-Side-B and recording/allocation observations at held PR #96 evidence `088226a...` are independently accepted. Clean held PR #112 at `15fbcfa...` supplies fresh PM-authenticated evidence and awaits blind exact-case Verification; all excluded behavior, source and complete-product acceptance remain held |
