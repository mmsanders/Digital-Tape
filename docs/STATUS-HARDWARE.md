...
**Updated:** 2026-08-31 · **Phase:** 0 · **Updated by:** Hardware Lead · **Reports to:** PM

The Hardware Lead's window into `hardware/`, `spec/thermal-budget.md` and the mechanical
track. Companion to `docs/STATUS.md`, which stays the Software Lead's. Same rules apply: what
changed, what is in flight, what is blocked and on what, what flipped to passing, and what
will hurt in three weeks. **Scope:** WP-04, WP-22–27, WP-29, WP-30, WP-34, WP-37.

---

## What changed since last time

**First report.** The Hardware Charter arrived 31 Aug and has been read against `CLAUDE.md`,
`docs/DECISIONS.md`, `docs/PACKAGES/README.md`, `spec/acceptance.md` and the Software Lead's
status report. Nothing has been built. This document is the review the PM asked for before
work starts.

**The environment was probed rather than assumed.** Results in *Blocked* below. Two of the
...
**`spec/` was not touched.** `spec/README.md` and `spec/thermal-budget.md` both still say "no
Hardware Lead assigned", which is now false, and `spec/thermal-budget.md` is a file the charter
assigns to me. Correcting either is escalation trigger #1, so neither was corrected. See H-11 —
this is a real ownership conflict, not a formality.
...
| Work | State |
|---|---|
| Charter review (this document) | Done |
| WP-04 transport spike packet | **Not started — blocked on H-03, H-04** |
| WP-34 `spec/thermal-budget.md` | **Not started — blocked on H-11, partially on H-02** |
| WP-05 parts order #1 | **Not started — blocked on H-02** |
| `hardware/` skeleton + build | Not started — blocked on H-03, and self-fixable |

Nothing is "in progress". I would rather report four honest zeros with named causes than start
the one package I could technically start and report movement.
...
Queued for Michael as **Q-004** with a stated default so it does not block. See also H-07 on
...
**Q-004** — the library printer: machine, materials, session limits, and how the queue really
works. Default if unanswered: PLA, 0.2 mm, ≤4 h plates, and I ship STL + 3MF + a plate picture
instead of G-code.

**Q-005** — a spend ceiling for hardware, and the figure above which I should ask before
proposing an order. Default if unanswered: I flag anything over $150 and assume nothing about
the three board spins.

---

## Acceptance criteria flipped to passing

None. Nothing has been built, and per H-12 there is currently nobody who could independently
...
- **The egress gap compounds silently.** Every day without it, I design against remembered part
  numbers instead of read datasheets, and the SGTL5000 question above is what that looks like on
  day one. The failure will not appear as a blocked task; it will appear as a schematic that
  looked fine until someone tried to buy it.
- **WP-34 is a Phase 0 package and it has not started**, for the reason in H-11, and the charter
  is explicit that the budget is written *before* the schematic so the schematic is a consequence
  of it. Every week it slips is a week in which the schematic gets designed against assumptions
  rather than a budget. This one needs an answer faster than the others.
- **The transport spike is the critical path and it has not left the building.** WP-04 blocks
  WP-20 and WP-22, and WP-22 is the six-to-ten-week long pole. The first library trip should
  happen this week and currently cannot, because I do not know what a printable file looks like
  for that printer (H-04). This is a one-question unblock and it is worth chasing Michael for.
- **H-01 wants settling before the format freeze, not after.** Q-002 is in front of Michael now
  and it is asking him to weigh a risk that I think is described incorrectly. If he answers "90
  minutes" on the current framing and the write floor turns out to bind, we will have frozen a
  format around a requirement no card specification guarantees — and unfreezing it costs a
  reflash of every cartridge ever written.
- **CI will be red for weeks and I am about to add to it.** ADR-005 anticipated hardware gates
  landing next to the software ones and noted the cost to reverse rises when they do. I would
  rather put hardware gates in a *separate* workflow that starts green and skips cleanly while
  `hardware/` is empty, so that the day a DRC violation appears it is the only red thing on the
  page. Flagging rather than deciding, since it touches the Software Lead's ADR.
